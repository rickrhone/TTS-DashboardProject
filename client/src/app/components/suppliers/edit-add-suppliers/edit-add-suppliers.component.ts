...
export class EditAddSuppliersComponent implements OnInit, OnDestroy {
      ...
      if (id) {
        this.suppliersService.get(id).subscribe((supplier: any) => {
          if (supplier) {
            ...
            console.log(`Supplier with id '${id}' not found, returning to the suppliers table`);
            this.resetForm();
          }
        });
      }
  ...
  insertRecord(form: NgForm) {
    ...
    this.suppliersService.postSupplier(form.value).subscribe(result => {
  ...
  updateRecord(form: NgForm) {
    ...
    this.suppliersService.putSupplier(form.value).subscribe(result => {
